...
function featuresHandler() {
    ...
    const featuresTextPlaceholder = document.getElementById("features__fonts__placeholder");
    featuresTextButton.addEventListener("click", (e) => {
        const featuresFontText = document.getElementById("features__fonts__text");
        const featuresFontSize = document.getElementById("features__fonts__size");
    ...
    const featuresIMG = document.getElementById("features__img");
    ...
    featuresIMGStart.addEventListener("click", (e) => {
        if (featuresIMGInterval)
            return;
        featuresIMGInterval = setInterval(() => {
            featuresIMG.style.top = `${Math.floor(Math.random() * (features.clientHeight + 1))}px`;
            featuresIMG.style.left = `${Math.floor(Math.random() * (features.clientWidth + 1))}px`;
        }, 1000);
    });
    ...
    const featuresChangePButton = document.getElementById("features__changeP__Btn");
    const featuresChangePElements = document.querySelectorAll("p");
    featuresChangePButton.addEventListener("click", (e) => {
        featuresChangePElements.forEach((element) => (element.style.fontSize = "15px"));
    });
    // Текстовий годинник
    ...
    //Поступове витирання
    const featuresRemoveDocument = document.getElementById("features__deletedocument");
    const featuresRemoveDocumentBG = document.querySelector(".features__deletedocumentbg");
    ...
    featuresTipButton.addEventListener("click", (e) => {
        ...
        document.addEventListener("click", (e) => {
            if (e.target === tip || e.target === featuresTipButton)
                return;
            tip.remove();
        });
    });
    ...
    const featuresColorSquare = document.querySelector(".features__colored__square");
    ...
    //Поточні координати
    const featuresCords = document.querySelector(".features__cords");
    ...
    featuresCordsStart.addEventListener("click", (e) => {
        if (featuresCordsTimer)
            return;
        featuresCordsTimer = setInterval(() => {
            featuresCords.innerHTML = `mouseX:${coords.mouseX}<br>mouseY:${coords.mouseY}<br>${coords.keyCode ? `keyCode:${coords.keyCode}` : ``}`;
        }, 50);
    });
    ...
    const featuresSortElements = document.querySelectorAll(".features__sort__element");
    ...
    // Не копiювати
    const featuresNoCopy = document.querySelector(".features__nocopy");
